...
export class MaintenanceTypesService {
  ...
  async create(dto: CreateMaintenanceTypeDto) {
    try {
      ...
      if ((error as { code?: string }).code === 'P2002') throw new ConflictException('Maintenance type already exists');
      throw error;
    }
  }
  ...
  async deactivate(id: string) {
    await this.ensureExists(id);
    return this.prisma.maintenanceType.update({ where: { id }, data: { isActive: false } });
  }

  private async ensureExists(id: string) {
    const type = await this.prisma.maintenanceType.findUnique({ where: { id } });
