<template>
  <div>
    <div class="mb-6 flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><p class="text-sm text-[#6b786f]">{{ greeting }}, {{ auth.user?.fullName }}</p><h2 class="display mt-2 text-2xl font-semibold tracking-tight">Bugünün görünümü</h2></div><span class="rounded-full bg-[#e4f0dd] px-3.5 py-1.5 text-sm font-semibold text-[#2f6b4f]">Canlı operasyon</span></div>
    <div v-if="pending" class="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><div v-for="item in 4" :key="item" class="h-32 animate-pulse rounded-2xl bg-[#e5ebe3]" /></div>
    ...
    <template v-else>
      <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <article v-for="card in cards" :key="card.label" class="panel p-4"><div class="flex items-start justify-between"><span class="text-sm text-[#68736d]">{{ card.label }}</span><span class="grid size-8 place-items-center rounded-xl text-sm" :class="card.color">{{ card.icon }}</span></div><strong class="display mt-4 block text-2xl">{{ card.value }}</strong></article>
      </div>

      <div class="mt-5 grid gap-4 xl:grid-cols-2">
        <section class="panel p-5">
          <div class="flex items-center justify-between">
            <div><p class="text-sm font-semibold text-[#2f6b4f]">Sonraki 7 gün</p><h3 class="display mt-1 text-lg font-semibold">Bakım yoğunluğu</h3></div>
            <span class="grid size-8 place-items-center rounded-xl bg-[#e4f0dd] text-[#2f6b4f] text-sm">◈</span>
          </div>
          <div class="mt-5 grid grid-cols-7 gap-2">
            <div v-for="day in upcomingByDay" :key="day.iso" class="flex flex-col items-center gap-1">
              ...
              <div class="flex h-24 w-full items-end justify-center">
                <div class="w-full max-w-[18px] rounded-t-md bg-[#2f6b4f] transition-all" :style="{ height: day.barHeight + 'px' }" />
              </div>
        ...
        <section class="panel p-5">
          <div class="flex items-center justify-between">
            <div><p class="text-sm font-semibold text-[#4d668f]">Son 30 gün</p><h3 class="display mt-1 text-lg font-semibold">Bakım türü dağılımı</h3></div>
            <span class="grid size-8 place-items-center rounded-xl bg-[#e8edf6] text-[#4d668f] text-sm">◌</span>
          </div>
          ...
          <div v-else class="mt-5 space-y-2.5">
            <div v-for="item in actionBreakdown" :key="item.typeId" class="flex items-center gap-3">
              ...
              <div class="h-2.5 flex-1 overflow-hidden rounded-full bg-[#eef1ec]">
...
const upcomingByDay = computed(() => {
  ...
  return raw.map((item) => ({
    ...
    barHeight: Math.max(6, Math.round((item.count / max) * 88)),
  }));
});
